...
def Trainer(model,  temporal_contr_model, model_optimizer, temp_cont_optimizer, train_dl, valid_dl, test_dl, device,
    ...
    """Pre-training for pre-training a model one labelled data, decided by the "training_mode" parameter."""
...
def model_pretrain(model, temporal_contr_model, model_optimizer, temp_cont_optimizer, train_loader, config,
                   device, training_mode, criterion = None, model_F = None, model_F_optimizer = None, epsilon = 1):
    ...
    for batch_idx, (data, labels, data_aug, data_f, data_f_aug) in enumerate(train_loader):
        ...
        # This is done using equation 3 from the paper, with epsilon as a parameter.
        loss_c = (loss_TF - loss_TF1 + epsilon) + (loss_TF - loss_TF2 + epsilon) + (loss_TF - loss_TF3 + epsilon)
