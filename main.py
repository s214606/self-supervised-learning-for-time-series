...
source_configs = SleepEEG_Config()
target_configs = Epilepsy_Config()
...
train_loader, valid_loader, test_loader = data_generator(sourcedata_path=sourcedata_path, targetdata_path=targetdata_path,
                                                         config = source_configs, augment=False)

TFC_model = TFC_Classifer(configs = source_configs)
classifier = target_classifier(source_configs).to(device = None)
